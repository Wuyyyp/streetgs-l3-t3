...
def training():
    ...
    from lib.utils.static_prune import scheduled_prune_iterations
    prune_iterations = scheduled_prune_iterations(optim_args, training_args.iterations)
    if prune_iterations:
        if not cfg.model.nsg.include_bkgd:
            raise ValueError("static pruning requires include_bkgd=True")
        from lib.utils.static_prune import load_importance_backend, prune_static_background
        load_importance_backend()
        print(f"Static background prune iterations: {sorted(prune_iterations)}")
    ...
    try:
        ...
        # Never silently continue with an incomplete checkpoint restore.
        if any(name.endswith('.pth') for name in os.listdir(cfg.trained_model_dir)):
            raise
    ...
    for iteration in range(start_iter, training_args.iterations + 1):
        ...
        if iteration in prune_iterations:
            prune_static_background(iteration, gaussians, scene.getTrainCameras())
        ...
        # pcd.points = o3d.utility.Vector3dVector(np.asarray(gaussians.background.get_xyz.cpu().detach()))
        ...
        # poses = [catRT(v.R.T, v.T) for v in viewpoint_stack]
        # from ysutils.util_open3d import gen_cam_frust
        # frusts = gen_cam_frust(poses,scale=0.02)
        # o3d.visualization.draw_geometries(frusts+[pcd])
        # # end


        # wys edit 0701
        mask = viewpoint_cam.guidance['mask'] if 'mask' in viewpoint_cam.guidance else torch.ones_like(gt_image[0:1]).bool()
        if data_args.type == 'M18':
            camtype_id = int(viewpoint_cam.image_name.split('_')[-1])
            mask_list = []
            if camtype_id == 3: # h,w
                leftup = (0,0)
        ...
        if 'obj_bound' not in viewpoint_cam.guidance:
            print('obj_bound not found')
        ...
        if optim_args.lambda_depth_lidar > 0 and lidar_depth is not None:            
            ...
            lidar_clip = optim_args.get('lidar_clip', None)
            if lidar_clip is not None:
                clip_near, clip_far = lidar_clip[0],lidar_clip[1]
                depth_mask = torch.logical_and(depth_mask, (lidar_depth>= clip_near) & (lidar_depth <= clip_far))
            expected_depth = depth / (render_pkg['acc'] + 1e-10)  
            depth_error = torch.abs((expected_depth[depth_mask] - lidar_depth[depth_mask]))
            # Some T3 cameras have no valid LiDAR returns after masking.
            if depth_error.numel() >= 2:
                ...
                loss += optim_args.lambda_depth_lidar * lidar_depth_loss *  (( 1 - iteration / training_args.iterations) * (1 - optim_args.lidar_weigth_decay) + optim_args.lidar_weigth_decay)
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:                    
                ...
                progress_bar.set_postfix({"NPoints": f"{(gaussians.background.get_xyz).shape[0]}",
            ...
            # Save resumable progress every 5,000 steps and at training completion.
            if iteration % 5000 == 0 or iteration == training_args.iterations:
