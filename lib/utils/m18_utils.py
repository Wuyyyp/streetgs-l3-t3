...
m18_track2label = { 'Car':0,'Pedestrian':1,'Non_motor_rider':2,'Barrier_gate':3,
 'Bicycle':4, 'Cone':5, 'Huge_vehicle':6, 'Motorcycle':7, 'Suv':8, "sign": 9,"Truck":10, "Bollards":11, "Sphere_bollards":12, 
"Bus":13,
"Crash_bucket":14,
"Vehicle_else":15,
"Police":16,
"Tricycle":17,"Stopping_sign":18,
"misc": -1,}

m18_if_deformable = {'Pedestrian':1,'Non_motor_rider':2,'Police':16}
...
img_intrin_check = [False,False,False,False,False,False,False,False,False,False,False]
...
def load_camera_info(datadir):
    ...
    intrins_f = glob(os.path.join(intrinsics_dir, '*.txt'))
    extrins_f = glob(os.path.join(extrinsics_dir, '*.txt'))
    egos_f = glob(os.path.join(ego_pose_dir, '*.txt'))

    intrins_f.sort()
    extrins_f.sort()
    egos_f.sort()

    # Calibration files must refer to the same images, including 7-camera L3.
    calibration_names = [os.path.basename(f) for f in intrins_f]
    if (calibration_names != [os.path.basename(f) for f in extrins_f]
            or calibration_names != [os.path.basename(f) for f in egos_f
                                     if '_' in os.path.basename(f)]):
        raise ValueError('Mismatched intrinsic, extrinsic, or camera pose filenames')

    intrinsics = [np.loadtxt(f) for f in intrins_f]
    extrinsics = [np.loadtxt(f) for f in extrins_f]
    ego_cam_poses = [np.loadtxt(f) for f in egos_f if '_' in os.path.basename(f)]
    ego_frame_poses = [np.loadtxt(f) for f in egos_f if '_' not in os.path.basename(f)]


    assert len(intrinsics) == len(extrinsics) and len(intrinsics) == len(ego_cam_poses), 'Mismatch between number of intrinsics, extrinsics, and ego poses'


    ncam = int(len(ego_cam_poses) / len(ego_frame_poses))
    ...
    # camp = gen_cam_frust(ego_cam_poses)
    # o3d.visualization.draw_geometries(camp)
...
def get_obj_pose_tracking(datadir, selected_frames, ego_poses, cameras=[0, 1, 2, 3, 4]):
    ...
    for tracklet in tracklets_str:
        ...
        if track_id not in objects_info.keys():
            ...
            try:
                objects_info[track_id]['class_label'] = m18_track2label[object_class]
            except:
                objects_info[track_id]['class_label'] = -1
    ...
    for key in objects_info.copy().keys():
        ...
        if len(all_obj_idx[0]) > 0:
            ...
            dynamic = np.any(np.std(obj_world_postions, axis=0) > 0.25) or distance > 0.1
            if not dynamic:
    ...
    for key in objects_info.keys():
        ...
        if obj['class'] in m18_if_deformable:
            obj['deformable'] = True
        else:
...
def generate_dataparser_outputs(
    ...
    image_filenames_all.sort()
    if not image_filenames_all:
        raise ValueError(f'No JPG images in {image_dir}')
    num_frames_all = max(image_filename_to_frame(os.path.basename(f))
                         for f in image_filenames_all) + 1
    ...
    # load calibration and ego pose, using ego pose mean to determine the center points, then
    # use the center points to modify the translation of the ego poses and camera poses
    intrinsics, extrinsics, ego_frame_poses, ego_cam_poses = load_camera_info(datadir)
    calibration_indices = {
        os.path.splitext(os.path.basename(filename))[0]: index
        for index, filename in enumerate(sorted(glob(os.path.join(datadir, 'intrinsics', '*.txt'))))
    }
    ...
    # for frame in range(start_frame, end_frame + 1):
    #     timestamp = timestamps[frame]['timestamp']
    #     assert timestamp.__str__()[-3:] == '000' and len(timestamp.__str__()) == 19, \
    #         f'wrong timestamp format: {timestamp.__str__()}'
    ...
    for image_filename in image_filenames_all:
        ...
        if frame >= start_frame and frame <= end_frame and cam in cameras:
            cam_idx = calibration_indices[os.path.splitext(image_basename)[0]]


            ixt = intrinsics[cam_idx]
            # ext = extrinsics[cam_idx]  # here defined as: cam to ego, just like the pose i think123456
            ext = extrinsics[cam_idx] #np.linalg.pinv(extrinsics[cam_idx])
            ...
            # timestamp = timestamps[frame]['cameras'][camera_name]['timestamp']
            # assert len(timestamp.__str__()) == 16, f'wrong timestamp format: {timestamp.__str__()}'
    ...
    # import open3d as o3d
    # from ysutils.util_open3d import gen_cam_frust
    # camp = gen_cam_frust(exts)
    # o3d.visualization.draw_geometries(camp)
    # camp = gen_cam_frust(c2ws)
    # o3d.visualization.draw_geometries(camp)
    ...
    # # test
    # import open3d as o3d
    # from ysutils.util_open3d import gen_cam_frust, create_bboxs
    # # check camera pose
    # cams_pos = gen_cam_frust(c2ws)
    # o3d.visualization.draw_geometries(cams_pos)
    # # check 3d bbox
    # sizes_bboxes = []
    # xyz_quat_bboxes = []
    # for idframe,framedata in enumerate(_):
    #     for idobj,objdata in enumerate(framedata):
    #         if objdata[0] > 0:
    #             idtrack = objdata[0]
    #             sizes_bboxes.append(np.asarray([object_info[idtrack]['height'], object_info[idtrack]['width'], object_info[idtrack]['length'] ]))
    #             xyz_quat_bboxes.append(objdata[1:])
    #
    # o3dobb = create_bboxs(sizes=sizes_bboxes, xyz_quats=xyz_quat_bboxes)
    # o3d.visualization.draw_geometries(o3dobb+cams_pos)
    # # end
    ...
    for i, image_filename in tqdm(enumerate(image_filenames)):
        cam = cams[i]
        if not img_intrin_check[cam]:
            cam_h, cam_w, _ = cv2.imread(image_filename).shape
            img_intrin_check[cam] = True
            image_heights[cam] = cam_h
            image_widths[cam] = cam_w
    ...
    os.makedirs(f'{cfg.model_path}/obj_bounds', exist_ok=True)
    for i, x in enumerate(obj_bounds):
        x = x.astype(np.uint8) * 255
        cv2.imwrite(f'{cfg.model_path}/obj_bounds/{i}.png', x)
    ...
    if not os.path.exists(os.path.join(colmap_basedir, 'triangulated/sparse/model')) and cfg.data.use_colmap:
        from script.M18.colmap_m18_full import run_colmap_m18
        run_colmap_m18(result)
    ...
    if build_pointcloud:
        ...
        # pcd.points = o3d.utility.Vector3dVector(pts3d_dict[0])
        ...
        for i, frame in tqdm(enumerate(range(start_frame, end_frame + 1))):
            ...
            for tracklet in object_tracklets_vehicle[i]:
                ...
                if track_id >= 0:
                    ...
                    # import open3d as o3d
                    # pcd = o3d.geometry.PointCloud()
                    # pcd.points = o3d.utility.Vector3dVector(points_xyz_obj)
                    # # o3d.visualization.draw_geometries([pcd])
                    #
                    # pcd2 = o3d.geometry.PointCloud()
                    # pcd2.points = o3d.utility.Vector3dVector(obj_corners_3d_local)
                    # pcd2.paint_uniform_color([1, 0, 0])
                    # # aabb = o3d.geometry.AxisAlignedBoundingBox.create_from_points(pcd2.points)
                    # obb = o3d.geometry.OrientedBoundingBox.create_from_points(pcd2.points)
                    # obb.color = [1, 0, 0]
        ...
        #     import open3d as o3d
        #     pcd = o3d.geometry.PointCloud()
        #     pcd.points = o3d.utility.Vector3dVector(points_xyz_dict['bkgd'][0])
        #     o3d.visualization.draw_geometries([pcd])
        #
        # pcd_all = o3d.geometry.PointCloud()
        # for i in range(0,2):
        #     points_lidar = o3d.geometry.PointCloud()
        #     points_lidar.points = o3d.utility.Vector3dVector(points_xyz_dict['bkgd'][i])
        ...
        #     pcd_all = pcd_all + downsample_points_lidar
        ...
        # Combine SfM and LiDAR by default. Pure-SfM experiments can disable
        # LiDAR initialization with data.use_lidar=false without changing the
        # behavior of existing configs.
        use_lidar = cfg.data.get('use_lidar', True)
        try:
            if cfg.data.filter_colmap:
                ...
                for i, ext in enumerate(exts):
                    ...
                    mask = np.logical_or(radius < cfg.data.get('extent', 10),
                                         points_colmap_xyz[:, 2] < camera_position[2])
            ...
            if use_lidar:
                ...
                print('pure SfM initialization: skip LiDAR background points')
        ...
        # import open3d as o3d
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(points_lidar_xyz)
        # # o3d.visualization.draw_geometries([pcd])
        #
        # pcd2 = o3d.geometry.PointCloud()
        # pcd2.points = o3d.utility.Vector3dVector(points_bkgd_xyz)
        # pcd2.paint_uniform_color((1,0,0))
