...
class ActorPose(nn.Module):      
    ...
    def load_state_dict(self, state_dict):
        if self.opt_track:
            ckpt = state_dict['params']
            for key in ['opt_trans', 'opt_rots']:
                if key in ckpt and ckpt[key].shape != getattr(self, key).shape:
                    ckpt_new = torch.zeros_like(getattr(self, key))
                    ckpt_new[..., :ckpt[key].shape[-2], :] = ckpt[key][..., :ckpt_new.shape[-2], :]
                    ckpt[key] = ckpt_new
