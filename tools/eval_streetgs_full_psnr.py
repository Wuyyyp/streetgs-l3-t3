...
assert (sample/'model/point_cloud'/f'iteration_{args.iteration}'/'point_cloud.ply').is_file()
...
sys.argv = ['full_psnr', '--config', str(sample/'train.yaml'), 'mode', 'evaluate',
            'loaded_iter', str(args.iteration), 'gpus', str([args.gpu]), 'data_device', 'cpu']
...
result = dict(sample=str(sample), iteration=args.iteration, image_count=707,
