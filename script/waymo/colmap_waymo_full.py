...
def run_colmap_waymo(result):    
    ...
    for unqiue_cam in unique_cams:
        train_images_dir = os.path.join(colmap_dir, 'train_imgs', f'cam_{unqiue_cam.__str__().zfill(1)}')
        test_images_dir = os.path.join(colmap_dir, 'test_imgs', f'cam_{unqiue_cam.__str__().zfill(1)}')
        mask_images_dir = os.path.join(colmap_dir, 'mask', f'cam_{unqiue_cam.__str__().zfill(1)}')
    ...
    for i in range(len(id_names)):
        ...
        rquat[0], rquat[1], rquat[2], rquat[3] = rquat[3], rquat[0], rquat[1], rquat[2]  # change to colmap format (w x y z)
