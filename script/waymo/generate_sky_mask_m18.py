...
def setup(args):
    ...
    def load_model_local(filename, ckpt_config_filename, device='cpu'):
        """
        从本地文件加载Grounding DINO模型

        Args:
            filename: 模型权重文件的本地路径
            ckpt_config_filename: 模型配置文件的本地路径
            device: 模型运行的设备
    ...
    # ckpt_file = "/home/yusen/Downloads/code/groundingdino_swinb_cogcoor.pth"
    # ckpt_config_file = "/home/yusen/Downloads/code/street_gaussians-main/submodules/GroundingDINO_SwinB.cfg.py"
    # groundingdino_model = load_model_local(ckpt_file, ckpt_config_file)
...
if __name__ == "__main__":
    ...
    if len(args.box_threshold) == 1:
        box_threshold = [args.box_threshold[0]] * 11
    else:
        assert len(args.box_threshold) == 11
