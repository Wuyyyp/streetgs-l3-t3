...
from lib.utils.m18_utils import load_camera_info
...
def convert_filename(filename):
    ...
    new_filename = f'cam_{cam_id}/{frame}.{ext}'
    return new_filename


def run_colmap_m18(result):
    ...
    for unqiue_cam in unique_cams:
        train_images_dir = os.path.join(colmap_dir, 'train_imgs', f'cam_{unqiue_cam.__str__().zfill(2)}')
        test_images_dir = os.path.join(colmap_dir, 'test_imgs', f'cam_{unqiue_cam.__str__().zfill(2)}')
        mask_images_dir = os.path.join(colmap_dir, 'mask', f'cam_{unqiue_cam.__str__().zfill(2)}')
    ...
    mask_ext = glob.glob(os.path.join(data_path, 'dynamic_mask/*'))[0].split('.')[-1]
    img_ext = image_filenames[0].split('.')[-1]
    ...
    for i, image_filename in enumerate(image_filenames):
        ...
        mask_basename = basename.split('.')[0] + f'.{mask_ext}'
        mask_image_filenames.append(os.path.join(data_path, 'dynamic_mask', mask_basename))
    ...
    for i, image_filename in enumerate(mask_image_filenames):
        ...
        mask_ext = new_mask_filename.split('.')[-1]
        new_mask_filename = new_mask_filename.replace(f'.{mask_ext}', f'.{img_ext}.{mask_ext}')
    ...
    for i in range(len(id_names)):
        ...
        transform = np.linalg.inv(transform)  # change pose to extrinsic

        r = R.from_matrix(transform[:3, :3])
        rquat = r.as_quat()  # The returned value is in scalar-last (x, y, z, w) format.
        rquat[0], rquat[1], rquat[2], rquat[3] = rquat[3], rquat[0], rquat[1], rquat[
            2]  # change to colmap format (w x y z)
    ...
    extrinsics = [np.linalg.pinv(e) for e in exts]
    # extrinsics defined in waymo/extrinsics datasets denote the pose of the camera in the world frame
    # here, we transform the true extrinsics to waymo 'extrinsics'
    ...
    #     --Mapper.tri_continue_max_angle_error 8')

    if cfg.data.use_colmap_pose:
        # May lead to unstable results when refining relative poses
        # Aborted (core dumped), we may use an additional method to refine the poses.
        raise ValueError('Aborted (core dumped), we should use an additional method to refine the poses.')
